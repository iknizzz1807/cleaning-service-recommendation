...
class EfficientNetClassifier(nn.Module):
    def __init__(
        self, model_name="efficientnet_b3", num_classes=10, pretrained=True, dropout=0.3
    ):
...
def classify_image(image, model):
    image = (
        data_transforms(image).unsqueeze(0).to(device)
...
def Inference(img: Image.Image, service_to_mess: dict):
    ...
    confs = dict()
    for service in service_to_mess:
        confs[service] = []
    len_services = dict()
    for service in service_to_mess:
        len_services[service] = 0
    ...
    for box in boxes:
        ...
        if confirmation == 1:
            continue
        class_name = predict_single_image_class(
            model, img.crop((x1, y1, x2, y2)), transform, classes, device
        )
        messes.add(class_name)
        for service in mess_to_service[class_name]:
            services.add(service)
            confs[service].append(conf)
        draw = ImageDraw.Draw(img)
        draw.rectangle([x1, y1, x2, y2], outline="red", width=3)
        font_size = 18
        font = ImageFont.truetype("arial.ttf", font_size)

        # Calculate text dimensions for blue box
        ...
        # Draw text on top of blue box
        draw.text((text_x, text_y), text, fill="white", font=font)
    service_and_conf = set()
    for service in services:
        avg_conf = sum(confs[service]) / len(confs[service])
        service_and_conf.add((service, avg_conf))
    return img, service_and_conf


if __name__ == "__main__":
    # Example usage
    img_path = (
        "./datanew/test/images/IMG_0613_png.rf.560fcc14458d84a5ba0a4703ab860ea7.jpg"
    )
